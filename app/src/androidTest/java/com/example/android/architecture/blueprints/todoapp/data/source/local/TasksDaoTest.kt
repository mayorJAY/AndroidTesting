...
class TasksDaoTest {
    ...
    fun initDb(){
        ...
        database = Room.inMemoryDatabaseBuilder(getApplicationContext(), ToDoDatabase::class.java).build()
    }
    ...
    fun insertTaskAndGetById() = runBlockingTest {
        ...
        assertThat<Task>(loaded, notNullValue())
    ...
    fun updateTaskAndGetById() = runBlockingTest{
        ...
        val task2 = Task("titles", "descriptions", true, task1.id)
        database.taskDao().updateTask(task2)

        // 3. Check that when you get the task by its ID, it has the updated values.
        val task3 = database.taskDao().getTaskById(task1.id)
        assertThat(task3?.id, `is`(task1.id))
        assertThat(task3?.title, `is`(task2.title))
        assertThat(task3?.description, `is`(task2.description))
        assertThat(task3?.isCompleted, `is`(true))
    }
}